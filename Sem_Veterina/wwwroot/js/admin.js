const convertKeysToLowerCase = (data) => {
    ...
    const fetchKlinikaDetails = async (id) => {
        try {
            const response = await fetch(`http://localhost:5240/api/Uzivatel/${id}`);
            if (!response.ok) {
                console.error('Chyba při načítání dat uzivatele:', response.statusText);
                return;
            }
    ...
    tableRows.forEach(row => {
        row.addEventListener('click', () => {
            ...
            fetchKlinikaDetails(id); // Zavolání API pro načtení detailu kliniky
        });
    });
